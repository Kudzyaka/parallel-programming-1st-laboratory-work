...
double fill_matrix_rows(int *a, size_t N, int fill_type) {
    ...
    // Режим 2 - Заполнение случайными (псевдослучайными) значениями
    else if (fill_type == 2) {
        start_time = omp_get_wtime();
        for (size_t i = 0; i < N; i++) {
            for (size_t j = 0; j < N; j++) {
                // Используем простую формулу от i, j, 
                // чтобы вызов функции rand() внутри не являлся узким горлышком.
                a[i * N + j] = (i * 31 + j * 17) % 1000;
            }
        }
        end_time = omp_get_wtime();
    } 
